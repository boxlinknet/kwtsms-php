...
class KwtSMSClientTest extends TestCase
{
    ...
    {
        $result = $this->client->validate(['96598765432', 'bad@email.com', '712345678']);
    ...
    public function testFromEnvLoadsCredentialsFromFile(): void
    {
        ...
        try {
            ...
            $this->assertSame('', $get('log_file')); // KWTSMS_LOG_FILE= (empty) must disable logging
        } finally {
            unlink($envFile);
    ...
    /**
     * KWTSMS_TEST_MODE string coercion: only '1', 'true', 'yes' enable test mode.
     * 'false', '0', '' must NOT enable it.
     *
     * @backupGlobals disabled
     * @dataProvider modeStringProvider
     */
    public function testFromEnvTestModeStringCoercion(string $envValue, bool $expected): void
    {
        $savedMode = getenv('KWTSMS_TEST_MODE');
        putenv('KWTSMS_TEST_MODE');
        unset($_ENV['KWTSMS_TEST_MODE']);

        $envFile = tempnam(sys_get_temp_dir(), 'kwtsms_env_');
        file_put_contents($envFile, "KWTSMS_USERNAME=u\nKWTSMS_PASSWORD=p\nKWTSMS_TEST_MODE={$envValue}\n");

        try {
            $client = KwtSMS::from_env($envFile);
            $prop = (new \ReflectionClass(KwtSMS::class))->getProperty('test_mode');
            $prop->setAccessible(true);
            $this->assertSame($expected, $prop->getValue($client), "KWTSMS_TEST_MODE={$envValue}");
        } finally {
            unlink($envFile);
            if ($savedMode !== false) {
                putenv("KWTSMS_TEST_MODE={$savedMode}");
                $_ENV['KWTSMS_TEST_MODE'] = $savedMode;
            }
        }
    }

    /** @return array<string, array{string, bool}> */
    public static function modeStringProvider(): array
    {
        return [
            'value 1 enables'       => ['1',     true],
            'value true enables'    => ['true',  true],
            'value yes enables'     => ['yes',   true],
            'value TRUE enables'    => ['TRUE',  true],
            'value false disables'  => ['false', false],  // was broken before fix: (bool)'false' === true
            'value 0 disables'      => ['0',     false],
            'value empty disables'  => ['',      false],
        ];
    }

    // --- Early credential check (v1.6.0) ---

    public function testSendReturnsEarlyOnEmptyUsername(): void
    {
        $client = new TrackingMockKwtSMS('', 'somepass', 'TEST', true, '');
        $result = $client->send('96598765432', 'Hello');

        $this->assertSame('ERROR', $result['result']);
        $this->assertSame('ERR003', $result['code']);
        $this->assertStringContainsString('Missing API credentials', $result['description']);
        $this->assertCount(0, $client->calls, 'post() should not be called with empty username');
    }

    public function testSendReturnsEarlyOnEmptyPassword(): void
    {
        $client = new TrackingMockKwtSMS('someuser', '', 'TEST', true, '');
        $result = $client->send('96598765432', 'Hello');

        $this->assertSame('ERROR', $result['result']);
        $this->assertSame('ERR003', $result['code']);
        $this->assertStringContainsString('Missing API credentials', $result['description']);
        $this->assertCount(0, $client->calls, 'post() should not be called with empty password');
    }

    public function testSendReturnsEarlyOnBothEmpty(): void
    {
        $client = new TrackingMockKwtSMS('', '', 'TEST', true, '');
        $result = $client->send('96598765432', 'Hello');

        $this->assertSame('ERROR', $result['result']);
        $this->assertSame('ERR003', $result['code']);
        $this->assertCount(0, $client->calls, 'post() should not be called with empty credentials');
    }
}
